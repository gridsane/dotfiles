...
local function markup(keys)
   ...
   for _, key in ipairs(keys) do
     ...
     result[group] = (result[group] or "") ..
        '<span font="DejaVu Sans Mono 10" color="' .. (beautiful.fg_widget_clock or '#FFF') .. '"> ' ..
        string.format("%" .. (longest - unilen(skey)) .. "s  ", "") .. skey ..
        '</span>  <span color="' .. (beautiful.fg_widget_value or '#FFF') .. '">' ..
        help .. '</span>\n'
      end
   end
...
function display()
   ...
   for group, res in pairs(strings) do
      ...
      result = result ..
     '<span weight="bold" color="' .. (beautiful.fg_widget_value_important or '#FFF') .. '">' ..
     group .. "</span>\n" .. res
   end
